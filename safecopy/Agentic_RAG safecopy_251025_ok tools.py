...
def bot_response(user_message: str, state: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any], ]:
    ...
    custom_prompt = """Sei un assistente intelligente in grado di utilizzare tool per rispondere alle domande: 
    ...
    custom_prompt2 = """Sei un assistente che può usare tool locali come /add, /weather, ecc."""
...
def build_demo():
    with gr.Blocks(title="Agentic RAG - tools + Ollama") as demo:
        ...
        with gr.Row():
            txt = gr.Textbox(label="Messaggio", placeholder="Scrivi qui la domanda")
