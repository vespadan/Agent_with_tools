...
def history_chain (query: str) -> str:
    """ Contesto preceedenti conversazioni """
    try:
...
def build_agent():
    ...
    history_tool = Tool(
        name="HISTORY",
        func=history_chain,
        description="Utilizza questo tool per acceder alal history delle precedenti conversazioni."
    )   
    
    # caricamento tools disponibili
    tools = [rag_tool, calc_tool, wiki_tool, weather_tool, history_tool]
    ...
    agent_kwargs = {
        ...
        "extra_prompt_messages": _memory_store
            }
...
def agentic_rag_interface(user_query, user_memory=None):
    ...
    if user_memory is None:
        user_memory = ConversationBufferMemory(memory_key="memory", return_messages=True)
