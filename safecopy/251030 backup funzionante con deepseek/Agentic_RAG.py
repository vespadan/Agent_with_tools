...
tool_names = ", ".join(f"/{name}" for name in sorted(tools.registered_functions.keys()))
...
Input Action: fornisci l'input per il tool. L'input deve essere JSON valido o una lista di coppie chiave=valore (es. key="val").
Observation: incolla qui il risultato restituito dal tool (questo campo è popolato dall'ambiente che esegue il tool).
...(Ripeti Thought / Action / Input Action / Observation al massimo 5 volte)
...
Struttura dell'invocazione tool: usa esattamente il formato Action e Input Action indicato più sopra; l'ambiente eseguirà il tool e fornirà Observation.
...
Limite iterazioni: non superare 5 iterazioni Thought / Action / Input Action / Observation per singola risposta; se non risolvi entro tale limite, torna indietro e fornisci una spiegazione parziale e i passi successivi consigliati.
...
Verifica la corretta formattazione della risposta. """)
...
def _make_langchain_tools():
    ...
    # creazione emebeddings dal testo fornito
    ...
    for name in sorted(list(tools.registered_functions.keys())):
        ...
        try:
            lc_tools.append(LC_Tool(name=name, func=make_fn(name), description=description))
        except Exception:
            # fallback to old Tool signature
            try:
                from langchain.agents import Tool as ToolOld

                lc_tools.append(ToolOld(name=name, func=make_fn(name), description=description))
            except Exception:
...
def bot_response(user_message: str, state: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], Dict[str, Any], str]:
    ...
    # If LangChain is available, build an agent that can use local tools and has a conversation memory.
    if LANGCHAIN_AVAILABLE:
        try:
            ...
            if "_AGENT_EXECUTOR" not in globals() or _AGENT_EXECUTOR is None:
                ...
                _AGENT_MEMORY = ConversationBufferMemory(memory_key="chat_history", return_messages=True)
