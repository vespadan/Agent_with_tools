...
# IMPORTANTE: Aggiungi LANGSMITH_API_KEY nel file .env invece di includerla nel codice
...
tool_names = ['/RAG', '/add', '/day_of_week', '/divide', '/multiply', '/square', '/subtract', '/today_is', '/weather', '/wikipedia', '/wolfram', '/recent_news']
...
'''
Your answers must be at least 500 words and directly address the user's query.
Carefully verify the accuracy of the material you've collected. Provide step-by-step explanations and examples where relevant. 
Avoid generating new questions or unnecessary information.
"""
...
LC_PROMPT = PREFIX + FORMAT_INSTRUCTIONS + SUFFIX
                ...
                if not args and (input_str or "").strip():
                    try:
                        ...
                        if fn:
                            ...
                            # Prefer explicit 'query' parameter if present.
                            if "query" in params:
                                args = {"query": input_str.strip()}
                            
                            # If the function takes a single meaningful parameter,
                            # pass the whole string as that param.
                            elif len(params) == 1:
                                args = {params[0]: input_str.strip()}
                            
                            # If it takes two parameters, try to split the input into
                            # two values using a comma or whitespace. If splitting
                            # doesn't produce two parts, fall back to duplicating
                            # the whole string for both parameters (best-effort).
                            elif len(params) == 2:
                                s = input_str.strip()
